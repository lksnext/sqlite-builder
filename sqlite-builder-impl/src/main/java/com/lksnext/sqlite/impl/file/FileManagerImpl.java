...
public class FileManagerImpl implements FileManager {
	...
	public void saveFile(byte[] content, URI uri) throws IOException {
		...
		OpenOption rewriteOption = Files.exists(path) ? StandardOpenOption.TRUNCATE_EXISTING
	...
	@Override
	public Boolean fileExists(URI uri) throws URISyntaxException {
	...
	public void computeMD5forFolder(URI sourceFolder, URI targetFolder) throws IOException {
		...
		for (String filename : filenames) {
			if (!"md5".equals(FilenameUtils.getExtension(filename))) {
				...
				String md5Filename = FilenameUtils.getBaseName(filename) + "-" + FilenameUtils.getExtension(filename)
						+ ".md5";
